...
# 4) HAVOK Reconstruction
    ...
    tau_steps = st.sidebar.number_input(
        "Time delay (steps of dt)", 1, max_steps, 30, 1, format="%d"
    )
    embed_dim = st.sidebar.number_input(
        "Embedding dimension m (≥3)", 3, 200, 10, 1, format="%d"
    )
    ...
    effective = N - warmup
    if effective < 1:
        st.error(f"Not enough data: need ≥{warmup+1} points, have {N}.")
        st.stop()

    # maximum SVD rank
    max_svd = min(n_delays, effective)
    svd_rank = st.sidebar.number_input(
        "SVD rank", 1, max_svd, max_svd, 1, format="%d"
    )
    ...
    model = pk.KoopmanContinuous(
        observables=TDC, differentiator=Diff, regressor=HAVOK
    )
...
# 5) EDMD Reconstruction
    ...
    svd_rank_edmd = st.sidebar.number_input(
        "EDMD SVD rank", 1, 3, 3, 1, format="%d"
    )
    ...
    # one-step predict + prepend initial state
    X1_pred = edmd_model.predict(X[:-1])
    X_pred  = np.vstack([X[0:1], X1_pred])
    ...
    # manual 3D delay‑embedding of x_pred
