...
# …[all the imports and first two modules unchanged]…
    ...
    poly = pk.observables.PolynomialLibrary(degree=3, include_bias=True)

    # — build & fit the EDMD model —
    edmd_model = pk.Koopman(
        observables=poly,
        regressor=pk.regression.EDMD(svd_rank=N-1)  # full rank by default
    )
    # fit on one‐step data
    edmd_model.fit(X[:-1], X[1:])

    # — simulate forward by repeatedly applying the EDMD map —
    #    this is equivalent to Colab's predict_evolution
    X_pred = edmd_model.predict_evolution(x_initial=X[0], time_points=t_eval)

    # — Plot the full (x,y,z) EDMD attractor —
