...
# ----------------------------------------------------------------
    ...
    # build & fit continuous‐time HAVOK
...
# ----------------------------------------------------------------
    ...
    max_idx = len(x_pred) - (d-1)*lag
    ED1 = x_pred[             :max_idx]
    ED2 = x_pred[     lag     :lag+max_idx]
    ED3 = x_pred[2*lag         :2*lag+max_idx]
    ...
    fig_e.update_layout(
        scene=dict(
            ...
            yaxis_title=f'x_pred(t+{lag}·dt)',
            zaxis_title=f'x_pred(t+{2*lag}·dt)'
        ),
        margin=dict(l=0, r=0, b=0, t=30)
    )
